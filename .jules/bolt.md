...
## 2026-06-08 - [Fast-path Sanitization with Early Truncation]
**Learning:** Using `str.isprintable()` provides a massive speedup (~2x) for "clean" strings by bypassing expensive regex processing. However, O(N) operations like `isprintable()` or regex searches must be preceded by early truncation to bound computational cost against malicious massive inputs and prevent potential DoS.
**Action:** Always perform length-bounding truncation before any O(N) string inspections or regex operations in high-frequency text processing paths.
